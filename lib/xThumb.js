...
var Grid = (function() {
	...
	function saveItemInfo( saveheight ) {
		$items.each( function() {
			...
			iData.set( 'offsetTop', $item.offset().top );
			if( saveheight ) {
				iData.set( 'height', $item.height() );
			}
		} );
	}
			...
			for(var c=0; c < dChildren.length; c++)
			{
				...
				if(matches(child, 'a'))
				{
					...
					child.addEventListener('click', function(e)
					{
						...
						current === getIndex($item.parentNode.children, $item) ? hidePreview() : showPreview( $item );
						return false;
	...
	function old_initItemEvents( $items ) {
	...
	function showPreview( $item ) {
		...
		preview = _data( this ).set( 'preview' , new Preview( $item ) );
	...
	Preview.prototype = {
		...
		update : function( $item ) {
			...
			var $itemEl = this.$item.children( 'a' ),
				eldata = {
					...
					largesrc : _data($itemEl).get( 'largesrc' ),
					title : _data($itemEl).get( 'title' ),
					description : _data($itemEl).get( 'description' )
				};
			...
			if( self.$fullimage.is( ':visible' ) ) {
				...
				$( '<img/>' ).load( function() {
					var $img = $( this );
					if( $img.attr( 'src' ) === _data(self.$item.children('a')).get( 'largesrc' ) ) {
						self.$loading.hide();
		...
		close : function() {
			...
			setTimeout( $.proxy( function() {
				...
				$expandedItem.css( 'height', _data($expandedItem).get( 'height' ) ).on( transEndEventName, onEndFn );
