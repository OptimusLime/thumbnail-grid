...
var Grid = (function() {
	...
	function _wrapClose(obj, delay, onEndFn)
	{
		setTimeout( function() {
			...
			if( typeof obj.$largeImg !== 'undefined' ) {
				...
				_fade.fadeOut(obj.$largeImg, 200);//.fadeOut( 'fast' );
			}
	...
	Preview.prototype = {
		...
		update : function( $item ) {
			...
			if( _selectCheck(self.$fullimage, ':visible' ) ) {
				_show(this.$loading);

				var nImg = document.createElement('img');
				nImg.setAttribute('src', eldata.largesrc);
				...
				nImg.addEventListener('load', function()
				{
					...
					if( $img.getAttribute( 'src' ) === _data(_firstChild(this.$item, 'a')).get( 'largesrc' ) ) {
